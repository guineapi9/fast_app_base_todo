...
class WriteTodoDialog extends HookConsumerDialogWidget<WriteTodoResult> {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final selectedDate = useState<DateTime>(DateTime.now()); //선택한 날짜를 저장
    final textController = useTextEditingController();
    final textController2 = useTextEditingController();

    useMemoized(() {
      if (todoForEdit != null) {
        //조건절은 useMemorized() 내부에 위치
        //수정할 때 값 입력. initState없이 사용 가능.
            ...
            children: [
              Row(
                children: [
                  ...
                  IconButton(
                      onPressed: wow, icon: const Icon(Icons.calendar_month))
                ],
              ),
              height20,
              Row(
